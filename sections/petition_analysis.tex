...
Given the list of petitions corresponding to campaigns on environmental and animal \pcm{I would potentially drop the animal welfare part as it is not introduced from the beginning of the paper} welfare issues on Twitter (described above), we first present an analysis on the petitions usage within different types of public campaigns and then analyse petition presence \pcm{??} outside of the campaign.
...
As described in \pcm{never write *the* before Section, Figure etc. with capital letters} Section~\ref{sec:dataset}, the campaign corpus is also annotated according to user engagement patterns for each campaign, and consists of four main types: one-day campaigns, ever-growing, annual, inactive.
We found that petitions were mainly promoted and tweeted by the \pcm{by the? reformulate} ``ever-growing'' campaigns (``\#saveafricananimals'', ``\#tweet4dolphins'' etc.).
...
On average, the tree has a relatively high branching uncertainty \pcm{?? factor?}, however, a few paths were more useful at predicting the petition success.
Unexpectedly, we observe that the higher the signature goal, $S(p)$, of a particular petition, the more likely it is to succeed \pcm{repetition, you talked about this above; I would shorten it above and focus here}. In particular, over 92\% of the petitions with $S(p)$ higher 100K obtained their required number of signatures.
% TODO: Add some motivation, examples etc...
Moreover, the higher the average number of followers a user has, $T_3(p)$, the less likely the petition is to attain the required number of signatures. \pcm{this is counter-intuitive, pls explain}
